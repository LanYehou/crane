...
blue_lower = np.array([100, 60, 60]) # 蓝色范围，最低值
blue_upper = np.array([124, 255, 255]) # 蓝色范围，最高值
red_lower = np.array([46, 43, 100]) # 红色范围，最低值
red_upper = np.array([255, 255, 124]) # 红色范围，最高值
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if color_x != 0 :
            ...
            if cricle_x != 0:
                print('redcricle',cricle_x,cricle_y)
            else:
                polygon_x,polygon_y,polygon_type=find_polygon()
                if polygon_type == 'rectangel' :
                    print('red'+polygon_type,polygon_x,polygon_y)
            ...
            if color_x != 0:
                cricle_x, cricle_y = find_circles()
                if cricle_x != 0:
                    print('bluecricle', cricle_x, cricle_y)
                else:
                    polygon_x, polygon_y, polygon_type = find_polygon()
                    if polygon_type == 'rectangel':
                        print('blue' + polygon_type, polygon_x, polygon_y)
                    elif polygon_type == 'triangel':
                        print('blue' + polygon_type, polygon_x, polygon_y)
                    else:
                        pass
